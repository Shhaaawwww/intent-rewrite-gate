...
`;
		...
		function apply(ctx) {
			...
			function IntentRewriteButton(props) {
				...
				const rewrite = async () => {
					...
					try {
						...
						if (result?.kind !== "success" || typeof result.text !== "string" || result.text.trim().length === 0) throw new Error("rewrite command failed");
			...
			}, (props) => (0, react.createElement)(IntentRewriteButton, props)));
		}
		//#endregion
		exports.apply = apply;
