...
const name = "intent-rewrite-gate";
const inject = [
	"llm",
	"commands",
	"agentDefaultModel"
...
async function rewriteOnce(ctx, draft, signal) {
	...
	if (finish !== "stop") throw new Error(`rewrite stopped with ${finish}`);
	const rewritten = blocks.join("\n\n").trim();
	if (rewritten.length === 0) throw new Error("rewrite returned no text");
	if (rewritten.length > outputLengthLimit(draft)) throw new Error("rewrite exceeded the conservative length limit");
	return rewritten;
}
function apply(ctx) {
	ctx.commands.register({
		...
		handler: async (invocation) => {
			...
			try {
				return {
					kind: "success",
					text: await rewriteOnce(ctx, draft, invocation.signal)
				};
			} catch (error) {
				...
				return {
					kind: "error",
					text: "intent rewrite failed; draft left unchanged"
				};
			}
		}
	});
